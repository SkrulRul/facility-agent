...
**Update (Phase 6):** the premise above ("no real persistence layer for this project") no longer holds once `DATABASE_URL` is set. `app/mcp_server.py`'s tools call the same `get_agreement_repository()`/`get_agreement_service()` DI providers as the HTTP API — both resolve to `PostgresAgreementRepository` instances hitting the same live database. Cross-process data sharing is no longer prevented by architecture; it is now a direct, deliberate consequence of backend selection (see ADR-0019). An MCP client can read live data created via the HTTP API in a separate process whenever `DATABASE_URL` is set. `scripts/mcp_server_seeded.py` still seeds directly into whatever backend `get_agreement_repository()` resolves to (in-memory by design when run without `DATABASE_URL`, matching its original purpose of exercising the stdio wire protocol, not proving Postgres durability — see `scripts/smoke_test_persistence.py` for that).
